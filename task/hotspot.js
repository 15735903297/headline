    ...
    request({
        ...
        async.eachLimit(arr, 1, (v, next) => {
            request({
                ...
                console.log(v);
                next(null);
                db.query("insert into hotspot(cid,title,dsc,image,url,create_time,content)values(?,?,?,?,?,?,?)",[1,v.title,v.dsc,v.image,v.url,' ',' '],function (err,rows,fields) {
                    if (err){
